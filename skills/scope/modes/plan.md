...
- Spawn a read only `researcher` subagent (capability first). `model`: the cheapest tier; do not inherit the session model (pi: the `researcher` type pins `haiku` and carries the web tools; a light model elsewhere) · `description: "Scope: verify reference links"`
- Tools: `read`, `web_search`, `fetch_content` (no `edit`; it does not write)
