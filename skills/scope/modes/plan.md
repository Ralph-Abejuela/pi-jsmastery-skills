...
- Spawn a read only web research subagent (capability first). `model`: the cheapest tier; do not inherit the session model (a light model that carries the web tools) · `description: "Scope: verify reference links"`
