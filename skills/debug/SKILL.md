...
- **Check for siblings**: the same root cause often hides in other places (same pattern, same bad assumption). Search for them with `grep` and note or fix them.
...
- `model`: set explicitly to a strong model, do not inherit the session model
