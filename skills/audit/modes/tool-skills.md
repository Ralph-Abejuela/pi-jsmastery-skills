...
- **Run it in the background, keep working.** Hand the set to a read only discovery subagent and do NOT wait on it. Set its model explicitly to a fast, low cost tier, never the session model (give the subagent the registry CLI plus web tools). It returns ONLY the compact candidate list, grouped by technology, minus installed and declined. Greenfield: start it right after the consent pick so searches overlap the standards questions; collect the result just before the offer. Fallbacks: no background support → same subagent, blocking; no subagent → search inline; no search capability → skip and note in the report. The offer panel stays on the main thread, since a subagent cannot prompt the engineer.
