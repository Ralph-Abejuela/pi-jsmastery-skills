...
Spawn a read only exploration subagent (capability first: a read only subagent type your client provides, such as Cursor `Explore`, Antigravity `research`, or Codex `spawn_agent`; else a plain subagent, or inline if your agent has none). Where the type does not pin it, set a fast, low cost model explicitly, never the session model. Tools `read`, `grep`, `find`, `bash` only, no `edit`/`write`. Brief: target workspace, exact sub task, the spec's key interfaces; return only a compact map, files to create/edit (paths), patterns/conventions to match (`file:line`), symbols/types/helpers to reuse, and gotchas, no file contents or dumps.
