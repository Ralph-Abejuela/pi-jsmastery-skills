...
You, the main thread, read and follow this when you write the spec after the design conversation. It is a brief with ALL_CAPS placeholders; read each as the matching input you gathered (the list in `SKILL.md`, *Write the spec*). You write it all yourself; the only subagents read the codebase or fetch the web on the cheapest model.
