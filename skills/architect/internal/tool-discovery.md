...
Then present a panel (capability first: your agent's picker, else the same options as plain text). Mark exactly one recommended. The picker appends its own free text option, so add one yourself only in a plain text fallback.
